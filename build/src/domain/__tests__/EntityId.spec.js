...
class TestId extends EntityId_1.EntityId {
}
describe('EntityId', () => {
    ...
    it('should be created with value', () => {
        ...
        expect(testId.toString()).toBe(`TestId(${idVal})-${testId.occuredDate.toISOString()}`);
    });
    it('should be immutable', () => {
        const expectedError = new TypeError(`Cannot assign to read only property 'occuredDate' of object '#<Object>'`);
        expect(() => {
        ...
        }).toThrow(new TypeError(`Cannot assign to read only property 'value' of object '#<Object>'`));
    });
});
